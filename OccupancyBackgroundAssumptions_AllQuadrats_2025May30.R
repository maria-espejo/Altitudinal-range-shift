...
# Examine the following background assumptions related to test predictions derived from hypotheses proposing that 
# Espeletia species from the páramo de Sumapaz are responding to global warming by expanding or leaning their geographic
...
#
# All sampled quadrats are considered to assess the occupancy of the species and their cohorts.
# 
...
# 4) Define younger and older cohorts.
# 5) Estimate mean, minimum and maximum elevation across occupied sampling cells, for the species or morphological
#    group and for younger and older cohorts.
# 6) Null model using all sampled cells.
# 7) Compare null model values with observed values.
# 8) Store the results for each species in a list when executed with a "for" loop for several species.
#
###################################################################################################################
###################################################################################################################
###################################################################################################################
...
###################################################################################################################
# 6.1) Prepare objects to run the null model.
###################################################################################################################
...
###################################################################################################################
# 6.3) Examine null model results.
###################################################################################################################
...
###################################################################################################################
# 8.1) store the results for each species in a list.
###################################################################################################################
...
###################################################################################################################
# 8.2) Store the summary of the results for each species in a data frame.
###################################################################################################################

# Espeletia "grandiflora" complex
gr.list.BA <- results.BA[["E. grandiflora"]]
grandiflora.results.BA <- data.frame(
...
write.csv(grandiflora.results.BA, file = "grandiflora.results.BA.allquadrats.csv")

# Espeletia killipii
ki.list.BA <- results.BA[["E. killipii"]]
killipii.results.BA <- data.frame(
  Predictions = c("LE", "ME", "UE"),
...
write.csv(killipii.results.BA, file = "killipii.results.BA.allquadrats.csv")


# Espeletia summapacis
su.list.BA <- results.BA[["E. summapacis"]]
summapacis.results.BA <- data.frame(
  Predictions = c("LE", "ME", "UE"),
...
write.csv(summapacis.results.BA, file = "summapacis.results.BA.allquadrats.csv")


# Espeletia "bractless" complex
br.list.BA <- results.BA[["complejo_sin_bracteas"]]
bractless.results.BA <- data.frame(
  Predictions = c("LE", "ME", "UE"),
...
write.csv(bractless.results.BA, file = "bractless.results.BA.allquadrats.csv")
