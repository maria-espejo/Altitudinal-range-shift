...
# Test predictions derived from the hypothesis proposing that Espeletia species from the páramo de Sumapaz are responding
...
# All sampled quadrats are considered.
#
...
# 5) Estimate mean, minimum and maximum elevation across occupied sampling cells, for each cohort.
# 6) Null model.
# 7) Compare null model values with observed values.
# 8) Store the results for each species in a list when executed with a "for" loop.
#
###################################################################################################################
###################################################################################################################
###################################################################################################################
...
#End of the "for" loop in section 8.1
...
grandiflora.results.disturbed <- data.frame(
...
write.csv(grandiflora.results.disturbed, file = "grandiflora.results.allquadrats.csv")

#Espeletia killipii
killipii.list <- results[["E. killipii"]]
killipii.results.disturbed <- data.frame(
  Predictions = c("LE", "ME", "UE"),
...
write.csv(killipii.results.disturbed, file = "killipii.results.allquadrats.csv")

#Espeletia summapacis
summapacis.list <- results[["E. summapacis"]]
summapacis.results.disturbed <- data.frame(
  Predictions = c("LE", "ME", "UE"),
...
write.csv(summapacis.results.disturbed, file = "summapacis.results.allquadrats.csv")

#Espeletia "bractless" complex
bractless.list <- results[["complejo_sin_bracteas"]]
bractless.results.disturbed <- data.frame(
  Predictions = c("LE", "ME", "UE"),
...
write.csv(bractless.results.disturbed , file = "bractless.results.allquadrats.csv")
